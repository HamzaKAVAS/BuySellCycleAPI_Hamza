Feature: [API_US028] As an administrator, I want to access the coupon details with the specified id number via API connection.
  ...
  and the contents of the couponDetails data (id, title, coupon_code, coupon_type, start_date, end_date, discount, discount_type, minimum_shopping, maximum_discount, created_by, updated_by, is_expire, is_multiple_buy, multiple_buy_limit, created_at, updated_at) in the response body must be verified.
    ...
    And The api user prepares a GET request body to send to the api holidayDetails endpoint containing the information <id>.
    ...
    Then The api user validates the contents of the holidayDetails data <id>, "<title>", "<coupon_code>", <coupon_type>,"<start_date>","<end_date>","<discount>","<discount_type>","<minimum_shopping>","<maximum_discount>","<created_by>","<updated_by>","<is_expire>","<is_multiple_buy>","<multiple_buy_limit>","<created_at>" and "<updated_at>" in the response body.
